...
assert.match(renderer, /view\.type === "course" && visibleItems\.length < 12/);
assert.match(renderer, /card\.setAttribute\("data-skeleton", "true"\)/);
assert.match(content, /onOrdinaryZoneRequest: \(type, event\) => \{[\s\S]*requestOrdinaryZoneFloor\(currentLifecycle, type, true, true\);/);
assert.doesNotMatch(content, /onOrdinaryZoneRequest: \(type, event\) => \{[\s\S]{0,320}requestOrdinaryZoneFloor\(currentLifecycle, type, true, false\)\.then/);
...
assert.match(renderer, /@media \(max-width: 1654px\)[\s\S]*ordinary-floor \.zone-list-box \{ width: 854px; grid-template-columns: repeat\(4, minmax\(0, 206px\)\)/);
assert.match(renderer, /const createOrdinarySkeletonItems = \(type, count = 12\)/);
assert.match(renderer, /\.ordinary-floor:not\(\.ordinary-pgc-floor\):not\(\[data-floor-id="course"\]\) \.video-card-common \.card-pic \.watch-later-video\.van-watchlater\.black \{\s*top: 8px;\s*right: 8px;\s*bottom: auto;/);
assert.match(renderer, /\.ordinary-floor:not\(\.ordinary-pgc-floor\):not\(\[data-floor-id="course"\]\) \.video-card-common \.card-pic \{\s*overflow: visible;/);
assert.match(renderer, /\.card-pic > \.card-link \{\s*overflow: hidden;\s*border-radius: 2px;/);
assert.match(renderer, /:not\(\.ordinary-pgc-floor\):not\(\[data-floor-id="course"\]\)/);
assert.match(renderer, /\.ordinary-floor:not\(\.ordinary-pgc-floor\):not\(\[data-floor-id="course"\]\) \.video-card-common \.card-pic \.count \{\s*bottom: 7px;\s*height: 16px;\s*min-height: 16px;\s*align-items: center;\s*padding: 0 8px;\s*background: none;\s*line-height: 16px;/);
assert.match(renderer, /\.card-pic \.count \.metric \{\s*display: flex;\s*height: 16px;\s*min-height: 16px;\s*align-items: center;\s*line-height: 16px;/);
assert.match(renderer, /\.card-pic \.count \.metric--duration \{\s*display: inline-flex;\s*align-items: center;\s*justify-content: flex-end;\s*line-height: 16px;/);
assert.match(renderer, /\.card-pic \.count \.right \{\s*width: auto;\s*min-width: 0;\s*height: 16px;\s*min-height: 16px;\s*margin: 0;\s*padding: 0;\s*flex: 0 0 auto;/);
assert.match(renderer, /\.card-pic \.count \.left \{\s*width: auto;\s*min-width: 0;\s*height: 16px;\s*min-height: 16px;\s*margin: 0;\s*padding: 0;\s*flex: 0 1 auto;/);
assert.match(renderer, /@keyframes watch-later-feedback-bounce/);
assert.match(renderer, /animation: watch-later-feedback-bounce \.26s ease-out/);
assert.match(renderer, /tips\.textContent = watchLater\.classList\.contains\("added"\) \? "移除" : "稍后再看"/);
