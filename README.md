...
- 可选的问题描述输入。
- JSON 诊断包和纯文本报告导出。
- 可选完整网页长截图；截图可能包含账号画面，用户可在导出前关闭。PNG 独立导出，诊断 JSON 只记录尺寸、分屏数和 SHA-256。
...
- `activeTab`：用户点击扩展图标后，可选逐屏捕获当前完整网页并在本地拼接成长图。
...
`0.2.69` 增加浅色/深色主题切换，完善完整网页诊断截图与登录态诊断证据，修复普通分区、直播、动态、稍后再看以及顶栏浮层的布局和交互。游戏中心与漫画顶栏组件改为本地 DOM 渲染，不再依赖跨域 iframe，并适配深色主题。

`0.2.68` 将普通视频分区调整为 `12/10/8/8` 响应式槽位，并修复完整网页截图必须从页顶开始的问题。BannerModel、本地 `.brbanner` 导入和轮换功能继续保留，Banner 包不包含在扩展本体中。
...
- 增加浅色与深色主题切换。
- 升级诊断导出功能，补充登录状态、接口阶段和页面运行状态等现场信息。
- 完善完整网页长截图，确保始终从页面顶部开始捕获。

### 修复

- 修复部分用户出现“登录状态未知”以及右上角功能缺失的问题。
- 修复普通视频分区在不同分辨率和缩放比例下的卡片数量与布局异常。
- 修复课堂分区在宽屏下的布局错位。
- 修复直播分区的卡片数量、封面预览和标题显示问题。
- 调整专栏列表的条目间距。
- 修复视频封面中的播放量、弹幕量和时长未水平对齐的问题。
- 修复“稍后再看”按钮的位置、提示文字和点击动效。
- 修复漫画分区“换一换”按钮缺少旋转动效的问题。
- 改善搜索历史、热搜悬浮项、排行榜和直播标题等内容在深色主题下的可读性。
- 修复顶栏主题选项布局，以及账号资产信息在深色主题下显示不清的问题。

### 已知问题

- 动态接口最多可返回 50 条视频，但当前组件最多只能完整渲染 20 条。
- 由于旧版本的游戏中心和漫画组件依赖跨域 iframe，无法完整适配深色主题；本版本暂时回退为本地渲染方案，部分样式和内容可能与官方组件存在差异。
